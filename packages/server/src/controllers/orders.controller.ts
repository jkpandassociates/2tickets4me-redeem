  ...
  async create(request: Request, reply: ReplyNoContinue) {
    ...
    try {
      const accessCode = await this._db.AccessCode.findOne({
        where: { Code: orderValues.CodeName }
      });

      orderValues.IPAddress =
      ...
      orderData['ConfirmationUrl'] = `http://${process.env.HOST}/order/${
      ...
      if (error instanceof Error) {
        ...
        switch (error.message) {
          case 'DUPLICATE':
            detail =
              'You are attempting to register multiple times.<br /><br />' +
...
export const orderRoutes = new OrdersController()
  .setDb(getDBContext())
